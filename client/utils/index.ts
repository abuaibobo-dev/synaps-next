...
/**
 * 获取后端 API 基础地址。
 * 优先使用环境变量 EXPO_PUBLIC_BACKEND_BASE_URL（云端部署时配置），
 * 未配置时默认指向 App 内嵌 Node 服务的本地地址。
 */
export function getApiBase(): string {
  const base = (process.env.EXPO_PUBLIC_BACKEND_BASE_URL ?? '').replace(/\/+$/, '');
  return base || 'http://127.0.0.1:19091';
}
